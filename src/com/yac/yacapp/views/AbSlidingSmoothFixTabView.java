...
public class AbSlidingSmoothFixTabView extends LinearLayout {
	...
	/** tab的列表. */
	private ArrayList<PopupTextView> tabItemList = null;
	/** tab的列表. */
//	private ArrayList<PopupTextView> tabItemList_pop = null;
	...
	/** tab滑块的高度. */
	private int tabSlidingHeight = 4;
	...
	public AbSlidingSmoothFixTabView(Context context, AttributeSet attrs) {
		...
		//定义Tab栏
		tabItemList = new ArrayList<PopupTextView>();
//		tabItemList_pop = new ArrayList<PopupTextView>();
		...
		view.setBackgroundColor(context.getResources().getColor(R.color.splite_line));
		this.addView(view,new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,2));
		//内容的View的适配
	...
	/**
	 * viewpager scroll controll
	 * @param scroll true:scroll  false:can't scroll
	 */
	...
	public class MyOnPageChangeListener implements OnPageChangeListener{
		...
		public void onPageSelected(int arg0) {
			//计算滑块偏移
			computeTabImg(arg0);
			//pagerItemList.get(arg0).onActivityCreated(null);
		}
	...
	public void addItemViews(List<String> tabTexts,List<Fragment> fragments){
		...
		for(int i=0;i<tabItemTextList.size();i++){
			...
			PopupTextView pop_tv = new PopupTextView(context);
			pop_tv.setTextColor(tabColor);
			pop_tv.setTextSize(tabTextSize);
			pop_tv.setText(text);
			pop_tv.setGravity(Gravity.CENTER);
			pop_tv.setLayoutParams(new LayoutParams(0,LayoutParams.FILL_PARENT,1));
			pop_tv.setPadding(0, 0, 0, 0);
			pop_tv.setFocusable(false);
			tabItemList.add(pop_tv);
			mTabLayout.addView(pop_tv);
			pop_tv.setOnClickListener(new OnClickListener() {
				@Override
...
//			});
	...
	public void updatePopupTextNum(int[] nums){
		if(nums.length == tabItemList.size()){//可以更新
			for(int i=0; i< nums.length; i++){
				if(nums[i] == 0 ){
					tabItemList.get(i).setTextNumVisibility(View.GONE);
				}else{
					tabItemList.get(i).setTextNumVisibility(View.VISIBLE);
					tabItemList.get(i).setTextNum(String.valueOf(nums[i]));
				}
			}
		}else{//不可以更新
			//TODO do something.
