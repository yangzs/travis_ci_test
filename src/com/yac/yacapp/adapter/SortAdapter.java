...
public class SortAdapter extends BaseAdapter implements SectionIndexer {
	...
	public View getView(final int position, View view, ViewGroup arg2) {
		...
		if (view == null) {
			...
			viewHolder.car_letter_tv = (TextView) view.findViewById(R.id.car_letter_tv);
		...
		if (position == getPositionForSection(section)) {
			viewHolder.car_letter_tv.setVisibility(View.VISIBLE);
			viewHolder.car_letter_tv.setText(mContent.first_letter);
		} else {
			viewHolder.car_letter_tv.setVisibility(View.GONE);
		}
		mImageLoader.displayImage(mContent.img_url, viewHolder.car_brand_img, options);
		viewHolder.car_brand_tv.setText(mContent.brand_name);

		return view;
	}

	final static class ViewHolder {
		TextView car_letter_tv;
	...
	public int getSectionForPosition(int position) {
		return list.get(position).first_letter.charAt(0);
	}
	...
	public int getPositionForSection(int section) {
		for (int i = 0; i < getCount(); i++) {
			String sortStr = list.get(i).first_letter;
